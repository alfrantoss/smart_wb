...
if(mysqli_num_rows($jikaada)>0){
	$exec = mysqli_query($koneksi, "UPDATE jawaban SET jawabmulti='$jawab',jawaban='$jawab' WHERE id_siswa='$id_siswa' AND id_ujian='$id_ujian' AND id_soal='$id_soal' AND jenis='$jenis'");
	}else{
$exec = mysqli_query($koneksi, "INSERT INTO jawaban (id_siswa,id_bank,id_soal,id_ujian,jawaban,jawabmulti,jenis) VALUES ('$id_siswa','$id_bank','$id_soal','$id_ujian','$jawab','$jawab','$jenis')");
...
$simpeun = mysqli_query($koneksi,"INSERT INTO jawaban_soal(id_soal,id_siswa,id_bank,id_ujian,idjawab,jenis,jawab) VALUES('$coba[id_soal]','$id_siswa','$id_bank','$id_ujian','$idjawab','3','$string')");
	}
}
