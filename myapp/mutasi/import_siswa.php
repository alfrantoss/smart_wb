...
if (isset($_FILES['file']['name'])) {
    ...
    if (in_array($extension, $ext)) {
        ...
        $sheetData = $spreadsheet->getActiveSheet()->toArray();
		 $sukses= 0;
		  mysqli_query($koneksi,"UPDATE pdb set jumlah='0'");
        for ($i = 4; $i < count($sheetData); $i++) {
            ...
            if ($ceknis == 0) {
                $result = mysqli_query($koneksi, "INSERT INTO siswa (nis,nisn,nama,level,kelas,jurusan,jk,agama,nowa,username,password,server) 
				VALUES ('$nis','$nisn','$nama','$level','$kelas','$pk','$jk','$agama','$nowa','$username','$pass','$server')");
            if($result){ 
			$sukses++;
			}
			}
      ...
      mysqli_query($koneksi,"UPDATE pdb set jumlah='$sukses',tahun='$tahun'");
    } else {
        echo "0";
    }
}
