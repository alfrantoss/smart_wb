...
<div class="col-md-4">
 ...
 <?php elseif($ac == 'login'): ?>
			...
			$level = $uji['level'];
			$pk = $uji['pk'];
			$sis = fetch($koneksi,'siswa',['level'=>$level,'sesi'=>$sesi,'jurusan'=>$pk]);
			$kelas = $sis['kelas'];
			$ruang = $sis['ruang'];
			?>
              <div class="row">
							...
							<div class="card-body">
									<div class="card-box table-responsive">
                                         <table id="datatable1" class="table table-bordered table-hover edis2" style="width:100%">
											...
											$no=0;
											if($pk=='semua'):
											$query = mysqli_query($koneksi,"SELECT * FROM siswa WHERE NOT EXISTS(SELECT * FROM nilai WHERE nilai.id_siswa=siswa.id_siswa and nilai.id_ujian='$_GET[idu]') and sesi='$sesi' and kelas='$kelas'");
											 else:
											 $query = mysqli_query($koneksi,"SELECT * FROM siswa WHERE NOT EXISTS(SELECT * FROM nilai WHERE nilai.id_siswa=siswa.id_siswa and nilai.id_ujian='$_GET[idu]') and sesi='$sesi' and kelas='$kelas' and jurusan='$pk'");
											 endif;
