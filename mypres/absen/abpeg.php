                         ...
                         <div class="col-xl-8">
						  <div class="card">
							...
							<h5 class="card-title bold">INPUT MANUAL</h5>
									...
									<div class="table-wrapper">
                                      <table class="table-siswa">
											...
											<?php
											$no=0;	
											$query = mysqli_query($koneksi,"SELECT * FROM users WHERE jabatan='$level' AND NOT EXISTS(SELECT * FROM absensi WHERE users.id_user=absensi.idpeg AND absensi.tanggal='$tanggal')");
											...
											?>
                                                ...
                                                <td><?= $no; ?> <input type='checkbox' name='idguru[]' value="<?= $data['id_user'] ?>"  ></td>                                  
                                                <td><?= $data['nip'] ?></td>
												<td><?= $data['nama'] ?></td>
												<td><?= $data['jabatan'] ?></td>							
												<td><label class='label-checkbox'><input type='radio' name='ket[]<?= $no; ?>' value="S"  ></label></td>
												<td><label class='label-checkbox'><input type='radio' name='ket[]<?= $no; ?>'value="I" ></label></td>
												<td><label class='label-checkbox'><input type='radio' name='ket[]<?= $no; ?>' value="A" ></label></td>
                                                </tr>
												<?php endwhile; ?>
												</tbody>
                                                </table>
					       ...
					       <div class="col-xl-4">
									...
									<div class="h5 mb-0"><?= $setting['sekolah'] ?></div>
										...
										<label class="bold">JABATAN</label>                               
										  <select class="form-select level">
												<?php $lev = mysqli_query($koneksi, "SELECT jabatan FROM users GROUP BY jabatan"); ?>
												<option value=''> Pilih Jabatan</option>
												<?php while ($kls = mysqli_fetch_array($lev)) : ?>
													<option <?php if ($level == $kls['jabatan']) {
													echo "selected";
													} else {
													} ?> value="<?= $kls['jabatan'] ?>"><?= strtoupper($kls['jabatan']) ?></option>
												<?php endwhile; ?>
												</select>													                                               
                                                </div>                                               
                                ...
                                var level = $('.level').val();
                                location.replace("?pg=<?= enkripsi('inpeg') ?>&level=" + level);
                                }); 
								</script>
								<script>
