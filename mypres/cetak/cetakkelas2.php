   ...
   <br>
    ...
    <table width="100%">
     ...
     <br>
		...
		$hadir= mysqli_num_rows(mysqli_query($koneksi, "SELECT * FROM absensi_les WHERE idsiswa='$siswa[id_siswa]' AND ket='H' AND bulan='$bulan' AND tahun='$tahun' "));
         $sakit= mysqli_num_rows(mysqli_query($koneksi, "SELECT * FROM absensi_les WHERE idsiswa='$siswa[id_siswa]' AND ket='S' AND bulan='$bulan' AND tahun='$tahun' "));
		 $izin= mysqli_num_rows(mysqli_query($koneksi, "SELECT * FROM absensi_les WHERE idsiswa='$siswa[id_siswa]' AND ket='I' AND bulan='$bulan' AND tahun='$tahun' "));
         $alpha= mysqli_num_rows(mysqli_query($koneksi, "SELECT * FROM absensi_les WHERE idsiswa='$siswa[id_siswa]' AND ket='A' AND bulan='$bulan' AND tahun='$tahun' "));
			  $no++;
			?>
				...
				$tanggal = cal_days_in_month(CAL_GREGORIAN, $bulan, $tahun);
                    for ($i = 1; $i < $tanggal + 1; $i++) { ?>
                        ...
                        $cekabsen = fetch($koneksi, 'absensi_les', ['tanggal' => $tanggalbaru, 'idsiswa' => $siswa['id_siswa']]);
                       if ($cekabsen) { ?>
...
$dompdf->render();
$dompdf->stream("Rekap Absen Eskul ".$eskul." Kelas ".$kelas." Bulan ". $bl . ".pdf", array("Attachment" => false));
exit(0);
?>