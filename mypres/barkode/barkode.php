									...
									<div class="card-box table-responsive">
                                        <table id="datatable1" class="table table-bordered table-hover" style="width:100%;font-size:12px">
                                           ...
                                           <tbody>
											<?php
											$no=0;
											$query = mysqli_query($koneksi, "SELECT * FROM datareg ORDER BY id DESC"); 
											while ($data = mysqli_fetch_array($query)) :
											$siswa = fetch($koneksi,'siswa',['id_siswa'=>$data['idsiswa']]);
											$peg = fetch($koneksi,'pegawai',['id_pegawai'=>$data['idpeg']]);
											$no++;
											   ?>
                                           <tr>
                                            <td><?= $no; ?></td>
                                ...
                                <div class="card">
									...
									<div class="h5 mb-0"><?= $setting['sekolah'] ?></div>
									   ...
									   <div class="d-flex justify-content-between mb-2">
										...
										<a href="?pg=<?= enkripsi('barkode') ?>&ac=<?= enkripsi('siswa') ?>" class="btn btn-success"><i class="material-icons">workspaces</i>Siswa</a>
										</div>
										<div class="text-center">
										 
										</div>
										<div class="text-center">
										<a href="?pg=<?= enkripsi('barkode') ?>&ac=<?= enkripsi('pegawai') ?>" class="btn btn-primary"><i class="material-icons">workspaces</i>Pegawai</a>
                                    </div>                    
									  </div>										
										 <div class="mb-4">
										...
										if (result.value) {
											$.ajax({
											   url: 'barkode/tbarkode.php?pg=hapus',
												...
												setTimeout(function() {
												window.location.replace('?pg=<?= enkripsi("barkode") ?>');
													}, 2000);
												}
											});
										}
      ...
      <?php elseif ($ac == enkripsi('siswa')) : ?>
                                        ...
                                        <table id="datatable1" class="table table-bordered table-hover" style="width:100%;font-size:12px">
                                           <thead>
											...
											$no++;
											  ...
											  <a href="?pg=<?= enkripsi('barkode') ?>&ac=<?= enkripsi('siswa') ?>&ids=<?= $data['id_siswa'] ?>"> <button class='btn btn-sm btn-success' data-bs-toggle="tooltip" data-bs-placement="top" title="Registrasi barkode"><i class="material-icons">edit</i></button></a>
											</td>
                                                </tr>
												<?php endwhile; ?>
                                                </table>
									...
									<div class="col-md-12 mb-1">
									  <label>Rombel</label>
                                       <input type='text' name='kelas' class='form-control' value="<?= $siswa['kelas'] ?>" readonly />
										...
										<label>Barcode</label>
									  <div class="col-md-12 mb-1" id="nobarkode">
					...
					$('#formkartu').submit(function(e){
						...
						$.ajax(
						{
							...
							setTimeout(function()
								{
								window.location.replace('?pg=<?= enkripsi("barkode") ?>&ac=<?= enkripsi("siswa") ?>');
										}, 2000);
		      ...
		      <?php elseif ($ac == enkripsi('pegawai')) : ?>			
									...
									<div class="card-box table-responsive">
                                        <table id="datatable1" class="table table-bordered table-hover" style="width:100%;font-size:12px">
										  ...
										  <tr>
											...
											<td>											
											  <a href="?pg=<?= enkripsi('barkode') ?>&ac=<?= enkripsi('pegawai') ?>&ids=<?= $data['id_user'] ?>"> <button class='btn btn-sm btn-success' data-bs-toggle="tooltip" data-bs-placement="top" title="Registrasi barkode"><i class="material-icons">edit</i></button></a>
											</td>
                                           </tr>
										<?php endwhile; ?>
										</tbody>
					...
					$('#formkarpeg').submit(function(e){
						...
						$.ajax(
						{
							...
							setTimeout(function()
								{
								window.location.replace('?pg=<?= enkripsi("barkode") ?>&ac=<?= enkripsi("pegawai") ?>');
										}, 2000);
