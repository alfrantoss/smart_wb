...
if($status['mode']=='5' AND $cek<>0):
$absen2 = mysqli_fetch_array(mysqli_query($koneksi, "SELECT * FROM absen_tu WHERE tanggal='$tanggal' ORDER BY id DESC LIMIT 1"));
$peg2 = mysqli_fetch_array(mysqli_query($koneksi, "SELECT * FROM users WHERE id_user='$absen2[idpeg]'"));
$notif_ronda = "Assalamualaikum wr. wb\n\nKami informasikan bahwa saudara\n\n*".$peg2['nama']."*\n\nHari ini tanggal ".$tanggal." telah hadir dalam kegiatan Piket Malam pada jam ".$jam."\nDemikian Informasi ini kami sampaikan agar menjadi monitoring Pegawai dan Kepala Sekolah\n\nWassalamualaikum wr.wb";
		$curl = curl_init();
		curl_setopt_array($curl, array(
		  CURLOPT_URL => $setting['url_api'].'/send-message',
		  CURLOPT_RETURNTRANSFER => true,
		  ...
		  CURLOPT_HTTP_VERSION => CURL_HTTP_VERSION_1_1,
		  CURLOPT_CUSTOMREQUEST => 'POST',
		  CURLOPT_POSTFIELDS => array('message' => $notif_ronda,'number' => $setting['nowa'])
		));
		curl_exec($curl);
		curl_close($curl);
		 sleep(1);
		...
		curl_setopt_array($curl, array(
		  ...
		  CURLOPT_HTTP_VERSION => CURL_HTTP_VERSION_1_1,
		  CURLOPT_CUSTOMREQUEST => 'POST',
		  CURLOPT_POSTFIELDS => array('message' => $notif_ronda,'number' => $peg2['nowa'])
		));
		curl_exec($curl);
		curl_close($curl);
